...
var daysOfWeek = ['monday',  'tuesday', 'wednesday', 'thrusday', 'friday', 'saturday', 'sunday'].sort();
...
var secondJedi = jedi[jedi.indexOf('Obi-Wan')];
jedi.splice(jedi.indexOf('Obi-Wan'),1);


// leave 'Mace Windu' in the array but put a copy of him on a var called thirdJedi

var thirdJedi = jedi.slice(jedi.indexOf('Mace Windu'),jedi.indexOf('Mace Windu')+1).toString();
...
// #10 Write a function called removeThings, that takes in an array of things (strings or numbers), 
//and a 'thing' (a string or a number).
...
function clockIn(empName, depName) {
	return {name : empName, department: depName, timestamp:new Date()};
}
// #14 Create a function called spiderTest that takes in an object that is an actor, 
//it looks to see if the actor's name is Andrew Garfield and changes the actor object 
//to have a property called spiderRating with a value of 10.  
//If their name is Tobey Maguire set spiderRating to 0.  
...
//a humidity(number), and a cloudiness(number), in that order. 
